...
export default class Horoscope {
  ...
  static calc(objDate: Date) {
    ...
    //五行
    json.wx =
      this.calcWxFw(json.nz).wx +
      '、' +
      this.calcWxFw(json.yz).wx +
      '、' +
      this.calcWxFw(json.rz).wx +
      '、' +
      this.calcWxFw(json.sz).wx;
    //方位
    json.fw =
      this.calcWxFw(json.nz).fw +
      '、' +
      this.calcWxFw(json.yz).fw +
      '、' +
      this.calcWxFw(json.rz).fw +
      '、' +
      this.calcWxFw(json.sz).fw;
    //十神
    json.shishen =
      this.calcShiShen(json.rz, json.nz) +
      '、' +
      this.calcShiShen(json.rz, json.yz) +
      '、日主、' +
      this.calcShiShen(json.rz, json.sz);
    return json;
  }
